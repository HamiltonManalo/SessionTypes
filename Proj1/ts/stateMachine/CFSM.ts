...
export class CommunicationStateMachine {
    ...
    public constructor(states: State[], webWorkerPath: string = null) {
        states.forEach(x => this.map[x.name] = x)

        this.worker = webWorkerPath != null ? new Worker(webWorkerPath) : null; 
    }

    public setStart = (start: stateName) => this._currentState = start; 

    public transit = (nextState: stateName) => {
        ...
        if(state.transitions.find(transition => transition.name == nextState) != undefined) {
            ...
            if(this.worker != null)
                newState.func.call(null, this.worker); 
            else 
